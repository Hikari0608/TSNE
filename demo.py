...
def data_random_crop(x,y,shape):
    px = []
    py = []
    ...
    for i,j in zip(x,y):
        ...
        imsize = i.shape[1:3]
        if(imsize[0]<=256 or imsize[1]<=256):
            px.append(tf.image.resize(i,size=shape,method='nearest'))
            py.append(tf.image.resize(j,size=shape,method='nearest'))
        else:
            [h,w] = np.random.randint(imsize-np.array(shape),size=2)

            px.append(i[:,h:h+shape[0],w:w+shape[1],:])
            py.append(j[:,h:h+shape[0],w:w+shape[1],:])
